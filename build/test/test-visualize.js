...
describe('toString', function () {
    it('should work for emitters', function () {
        ...
        expect(e.toString()).to.equal('| manual | 0 |>');
    });
...
describe('object inspector', function () {
    describe('getReceivers', function () {
        it('should give empty array for not plugged emitter', function () {
            var e = electric.e.manual(0);
            expect(v.inspector.getReceivers(e)).to.eql([]);
        });
        it("should give null for objects that don't have receivers variable", function () {
            var o = {};
            expect(v.inspector.getReceivers(o)).to.equal(null);
        });
        it('should return function receiver', function () {
            var e = electric.e.manual(0);
            e.plugReceiver(double);
            expect(v.inspector.getReceivers(e)).to.eql([{ name: '<| double |', value: double }]);
        });
        it('should return transformator receivers', function () {
            var e = electric.e.manual(0);
            var t = e.map(function (x) { return 2 * x; });
            expect(v.inspector.getReceivers(e)).to.eql([{ name: '<| map(=>) | 0 |>', value: t }]);
        });
    });
    describe('gerEmitter', function () {
        it('should work with emitters', function () {
            var e = electric.e.manual(0);
            var t = e.map(function (x) { return 2 * x; });
            expect(v.inspector.getEmitters(t)).to.eql([{ name: '| manual | 0 |>', value: e }]);
        });
        it('should work with transformators', function () {
            var e = electric.e.manual(0);
            var t0 = e.map(function (x) { return 2 * x; });
            var t1 = t0.filter(0, function (x) { return x < 2; });
            expect(v.inspector.getEmitters(t1)).to.eql([{ name: '<| map(=>) | 0 |>', value: t0 }]);
        });
    });
});
describe.only('walk', function () {
    it('should construct graph with single node', function () {
        var e = electric.e.manual(0);
        expect(v.Graph.of(e).nodesById).to.eql({ '0': { id: 0, name: '| manual | 0 |>', receivers: [], emitters: [] } });
    });
    it('should construct graph with two nodesById from emitter', function () {
        var e = electric.e.manual(1);
        var t = e.map(double);
        expect(v.Graph.of(e).nodesById).to.eql({
    ...
    it('should construct graph with two nodesById from receiver', function () {
        var e = electric.e.manual(1);
        var t = e.map(double);
        expect(v.Graph.of(t).nodesById).to.eql({
    ...
    it('should work with recursion', function () {
        var collision = electric.e.placeholder(electric.event.notHappend);
        var bullets = electric.e.constant('s').change({
            to: electric.e.constant('w'),
            when: collision
        });
        collision.is(bullets.whenThen(function (x) { return x; }));
        bullets.plugReceiver(function (x) { return x; });
