...
},{"./emitter":6,"./scheduler":9,"./transformator":10}],2:[function(require,module,exports){
...
},{"./emitter":6}],3:[function(require,module,exports){
...
},{"../electric":5}],4:[function(require,module,exports){
var electric = require('../electric');
...
},{"../electric":5}],5:[function(require,module,exports){
exports.scheduler = require('./scheduler');
...
},{"./clock":1,"./device":2,"./emitter":6,"./fp":7,"./receiver":8,"./scheduler":9,"./transformator":10}],6:[function(require,module,exports){
...
},{"./transformator":10}],9:[function(require,module,exports){
...
},{"./emitter":6,"./scheduler":9}],11:[function(require,module,exports){
/// <reference path="../d/chai.d.ts" />
/// <reference path="../d/mocha.d.ts" />
var expect = chai.expect;
var io = require('socket.io-client');
var rrc = require('../src/devices/requestResponseClient');
var electric = require('../src/electric');
var electricSocket = require('../src/devices/electric-socket');
function createPluggedDevice(method, url, maker) {
    if (maker === void 0) { maker = rrc.requestResponse; }
...
describe('request response client device', function () {
    it('should emit response data', function (done) {
        var pluggedDevice = createPluggedDevice('POST', 'http://localhost:8081');
        var responses = pluggedDevice.responses.map(function (x) { return x.data; });
        expect(responses)
            .to.emit(null)
            .after(function () {
    ...
    it('should emit response success (200) status', function (done) {
        var pluggedDevice = createPluggedDevice('POST', 'http://localhost:8081');
        var responses = pluggedDevice.responses.map(function (x) { return x.statusCode; });
        expect(responses)
            .to.emit(-1)
            .after(function () {
    ...
    it('should emit response not found (404) status', function (done) {
        var pluggedDevice = createPluggedDevice('POST', 'http://localhost:8080/fdfsdds');
        var responses = pluggedDevice.responses.map(function (x) { return x.statusCode; });
        expect(responses)
            .to.emit(-1)
            .after(function () {
    ...
    it('should emit state of request (not the http one)', function (done) {
        ...
        expect(pluggedDevice.state)
            .to.emit('none')
            .then.after(function () { return pluggedDevice.data.emit('t5'); })
            .to.emit('waiting')
            .to.emit('success')
            .then.finish(done);
    });
    it('should emit `error` state of request (not the http one)', function (done) {
        var badPluggedDevice = createPluggedDevice('POST', 'http://localhost:8080/fdsfsd');
        expect(badPluggedDevice.state)
            .to.emit('none')
            .then.after(function () { return badPluggedDevice.data.emit('t6'); })
            .to.emit('waiting')
            .to.emit('error')
            .then.finish(done);
    });
...
describe('JSON request response client device', function () {
    it('should encode and decode data', function (done) {
        var pluggedDevice = createPluggedDevice('POST', 'http://localhost:8081/json', rrc.JSONRequestResponse);
        expect(pluggedDevice.responses.map(function (x) { return x.data; }))
            .to.emit(null)
            .then.after(function () { return pluggedDevice.data.emit({ x: 1 }); })
            .to.emit({ ok: "{\"x\":1}" })
            .then.finish(done);
    });
});
describe('optimistic request response client device', function () {
    it('should emit optimistic version', function (done) {
        ...
        var pluggedDevice = createPluggedDevice('POST', 'http://localhost:8081', function (m, u) { return rrc.OptimisticRequestResponse(m, u, o); });
        expect(pluggedDevice.responses.map(function (x) { return x.data; }))
            .to.emit(null)
            .then.after(function () { return pluggedDevice.data.emit('o1'); })
            .to.emit(o('o1'))
            .then.finish(done);
    });
    it('should emit optimistic version and then correction', function (done) {
        ...
        expect(pluggedDevice.responses.map(function (x) { return x.data; }))
            .to.emit(null)
            .then.after(function () { return pluggedDevice.data.emit('o3'); })
            .to.emit(o('o3'))
            .to.emit('OK: o3')
            .then.finish(done);
    });
    it('should emit optimistic version and then error', function (done) {
        ...
        var pluggedDevice = createPluggedDevice('POST', 'http://localhost:8080/fdfsd', function (m, u) { return rrc.OptimisticRequestResponse(m, u, o); });
        expect(pluggedDevice.responses.map(function (x) { return x.statusCode; }))
            .to.emit(-1)
            .then.after(function () { return pluggedDevice.data.emit('o4'); })
            .to.emit(200)
            .to.emit(404)
            .then.finish(done);
    });
...
describe('optimistic JSON request response client device', function () {
    it('should (d)encode data and use custom equality (false)', function (done) {
        ...
        var pluggedDevice = createPluggedDevice('POST', 'http://localhost:8081/json', function (m, u) { return rrc.OptimisticJSONRequestResponse(m, u, o, e); });
        expect(pluggedDevice.responses.map(function (x) { return x.data; }))
            .to.emit(null)
            .then.after(function () { return pluggedDevice.data.emit({ x: 1 }); })
            .to.emit({ ok: "{\"x\":1}" })
...
emitter.plugReceiver(socketReceiver('toserver', socket));
// emitter.emit('x1');
...
},{"../src/devices/electric-socket":3,"../src/devices/requestResponseClient":4,"../src/electric":5,"socket.io-client":12}],12:[function(require,module,exports){
